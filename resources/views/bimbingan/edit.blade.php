            ...
            <form action="{{ route('bimbingan.update', $bimbingan) }}" method="POST">
                        ...
                        @if ($tugasAkhirs->count() > 0)
                            @foreach ($tugasAkhirs as $tugasAkhir)
                                <option value="{{ $tugasAkhir->id }}"
                                        ...
                                        {{ old('tugas_akhir_id', $bimbingan->tugas_akhir_id) == $tugasAkhir->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="mahasiswa_id" id="mahasiswa_id" class="form-control select2 @error('mahasiswa_id') is-invalid @enderror" required>
                        ...
                        <option value="{{ $bimbingan->mahasiswa_id }}" selected>{{ $bimbingan->mahasiswa->user->nama }}</option>
                    </select>
                    @error('mahasiswa_id')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea name="catatan" id="catatan" class="form-control @error('catatan') is-invalid @enderror" rows="5" required>{{ old('catatan', $bimbingan->catatan) }}</textarea>
                    @error('catatan')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal" id="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $bimbingan->tanggal->format('Y-m-d')) }}" required>
                    @error('tanggal')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            // Trigger change on page load to populate mahasiswa_id
